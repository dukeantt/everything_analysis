...
hard_code_word_map_og = json.load(open("data/hard_code_word_map_json.json", encoding='utf-8-sig'))
hard_code_word_map = {}
...
with open('data/begin.pkl', 'rb') as word:
    begin_prob = pickle.load(word)
begin_prob = dict(begin_prob)

with open('data/word_prob.pkl', 'rb') as word:
...
def dict_generate(abbreviation, telex):
    ...
    with open("./data/ignore_words.txt", "r", encoding='utf-8-sig') as ignore:
...
def do_correction(input_sentence):
    # Todo Main
    """main"""
    abbreviation_file = "./data/abbreviation_json.json"
    telex_file = "./data/telex_typing_json.json"
